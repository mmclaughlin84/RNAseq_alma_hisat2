...
for sample_name in ${sample_names[@]}; do
	sbatch --time=02:00:00 --job-name=$sample_name --partition=compute --mail-type=END,FAIL --mail-user=user_name@icr.ac.uk --cpus-per-task=8 --mem-per-cpu=1000 --output=log4_trim_${sample_name}.txt \
