...
The final step is to go to the RDS project folder, and open scripts 2-8 in **Textmate**. *There are two instances where you need to change "user_name" to your name (ie if John Smith this will be jsmith). Using find and replace (cmd-f/ctrl-f), find user_name and replace with your login user_name that corresponds to your email address and your folder on the RDS and created in the ALMA scratch location for your team (ie /TEAMNAME/user_name)* - sending emails to the default email will not be of much use to you, nor will trying to transfer files to a folder called user_name.
